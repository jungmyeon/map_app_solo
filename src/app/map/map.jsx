...
export default function KakaoMap() {
  ...
  const calculateRoute = useCallback((startPosition, endPosition) => {
    ...
    axios
      .get(
        `https://apis-navi.kakaomobility.com/v1/directions?origin=${origin}&destination=${destination}&waypoints=&priority=RECOMMEND`,
        {
          headers: {
  ...
  return (
    <div>
      <div className="SearchBar">
        <button className="Search-button1" onClick={handleShowRoadmap}>
          길찾기
        </button>
        ...
        <button onClick={moveToCurrentLocation} className="my-current-location">
          {" "}
          &#10146;
        </button>
        {showRoadmap && (
              ...
              {routeInfo && detailRoads.length > 0 && (
                ...
                >
                  <div className="route-info">
                    ...
                    <br />
                    <span>
                      시간 : {(routeInfo.duration / 60).toFixed(2)} 분
                    </span>
                  </div>
                </CustomOverlayMap>
              )}
